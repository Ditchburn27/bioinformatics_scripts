...
def EasySci_count_parallel(gtf_file, input_folder, output_folder, sample_ID_file, core_number):
  ...
  # edited function to find exons in gene gtf file, rather than creating an exon gtf file. 26.05.2025
    for feature in gtf_file:
        if feature.type == "exon":
            gene_n +=1
            genes[ feature.iv ] += feature.attr["exon_id"]
            gene_count += 1

            message = (feature.attr["exon_id"] + "," + feature.attr["gene_type"] + "," 
                        + feature.attr["gene_name"] + "," + feature.attr["exon_number"] + "," + str(gene_count) + "\n")
